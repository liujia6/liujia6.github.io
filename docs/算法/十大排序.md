...
- 通过循环从左到右比较**相邻的两个元素大小**，如果不满足排序条件就**交换**位置.
- 每次循环都能把第 i 大的元素放到正确的位置，**最大的元素像冒泡一样浮动到数组末尾**（数组右侧循环逐渐排序正确）
...
- 特点：
	- 对于部分有序数组来说很高效
	- 稳定排序（因为只有大于时才交换）
		- 值相等的两个元素，排序后**先后顺序保持和原数组一致 = 稳定**
	- 时间复杂度：等差数列 n + (n - 1) + (n - 2) + 1 ，
