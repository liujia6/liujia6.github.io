...
- JS 使用 IEEE 754 64 位双精度 `二进制浮点数` 存储数字
- 八进制
	- 首位为 0
...
- 处理数字统一性的以及比较的库
- ECMAScript 使用 IEEE 754 格式表示整数和浮点数（double），使用 64 位固定长度来表示
	- 为什么 0.1 + 0.2 !== 0.3
	- 由于 0.1 和 0.2 转二进制是 `无限循环小数`，计算机只能截断存储，产生微小精度误差。所以
	- 规避方法
			- 比较用 Number.EPSILON
...
- 如果 类型相同，直接走 === 判断逻辑
- 如果两个值的类型不一样
	- null 和 undefined 和任何其他数都不约等，只互相或者和自己==
	- 原始类型（number/string/boolean）的数据最终会全部转换成数值类型再进行比较。
	- 如果一个操作数是对象，另一个操作数不是则先使用 symbol.toPrimitive(default)
		- 先调用 valueOf，后 toString
	- 特殊规则
		- 任何数和 NaN 都不相等
		- `null == undefined //true`
		- `+0 == -0//true 且 +0 === -0 → true  
		- `NaN === NaN // false`
		- null / undefined 只有自己 === 自己才是 true
...
1. 调用 obj\[Symbol.toPrimitive](hint)；
	1. 如果 hint 取值是 "string"：
		- 首先返回 toString（）值，如果不是原始类型的值，继续调用 value() 方法，如果是原始类型的值就返回 Number（）值就结束。
...
3. 预期为数值的地方，除了加法运算符（+）有可能把运算子转为字符串，其他运算符都会把运算自动转成数值。
