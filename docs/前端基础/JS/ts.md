...
- interface 主要用来定义对象，然后可配合 class implements 来约束类的结构以及 extends 继承；然后支持声明合并，可以跨文件原地扩展；
- type 是通用类型别名工具，虽然也可以描述对象，但核心优势是支持联合、交叉、元组等类型运算，不支持同名原地扩展原有类型。
- 纯对象结构优先 interface，只有需要联合|、元组&、交叉分支等类型运算时才使用 type；

type 别名的优势

- 当类型需要复用，或者类型名称过长时使用 type 别名。对于简单的对象类型，可以根据团队习惯选择 type 或接口
- 可以给任何复杂类型定义一个别名，下次用起来不用重复写

interface 相比 type 独有的核心能力：

1. 同名自动合并，适合全局 / 第三方扩展，可扩展 Window/DOM 等全局内置对象；
2. 支持多继承 `extends`、类 `implements` 标准契约；
4. 直观支持函数、构造函数多重载签名。（了解即可）

## 泛型

让类型也拥有类似函数参数的能力。

类型参数化， 保留类型关系，在定义函数、组件或工具类型时不提前固定具体类型，而是在使用时确定类型。它的主要作用是在实现代码复用的同时，保持输入输出之间的类型关联，让 TypeScript 仍然能够进行类型推导。
...
// Anders 借鉴 C# 的泛型设计，引入类型占位符 `<T>`，一套逻辑复用，调用时自动推导真实类型；
//泛型接口
interface configFn{
    <T>(value:T):T;
}
