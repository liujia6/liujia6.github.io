...
闭包就是内层函数引用了外层函数的变量，并且内层函数在外部执行。

它会留住外层的变量不被销毁，可以用来封装私有数据；缺点是长期占用内存，不用时手动清空引用就行。

我们使用闭包时需要关注它的引用方，防止页面或组件不再使用闭包后，内存还无法释放，造成内存泄漏。如果闭包绑定了定时器、DOM 事件、全局监听，就要在组件卸载时主动清理引用，方便 GC 回收。
...
- 默认绑定
	...
	- 普通函数直接调用（全局 / 严格模式 undefined）
- 箭头函数：不绑定 this，继承外层作用域 this
...
- \_\_proro\_\_
	...
	- 原型链
		...
		- 在函数内部查找某个属性和方法时会沿着它的原型链查找，具体是查找它的*proto*指向的原型对象，默认是指向它的 prototype 即 `obj.__proto.__=obj.prototype`，如果 proto 指向被修改，它会去找这个被修改了的对象而不是它的 prototype，
- constructor
	- __constructor 是显式原型 prototype 的一个属性__ 也就是说只能通过 `obj.prototype.contructor` 来访问,它指向原型的构造函数
...
这道题其实思路很多，但是比较简单的方法我感觉是先把这个数转化为二进制，如果是 2 的整数次方，那么第一位是 1，其余都是 0。如果满足了这个，那么这个数就是 2 的整数次方
