...
- **宏任务**：包括整体代码 script，setTimeout，setInterval，MessageChannel，还有用户的一些操作的回调，如鼠标点击事件，键盘事件，ajax 请求，dom 操作以及 ui 渲染
- **微任务**：Promise.then(非 new Promise)，process.nextTick(node 中)、MutationObserver(html5 新特性)，`queueMicrotask`（`queueMicrotask` 是一个语义更明确，但功能更弱、兼容性更差、没有 await、没有 catch 的 “残疾版 Promise.then”）
