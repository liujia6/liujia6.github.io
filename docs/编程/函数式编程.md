...
- 偏函数 ([partial](https://www.lodashjs.com/docs/lodash.partial)) : 类似于 bind，固定一个函数的部分参数
...
- **柯里化（标准）**
	转换目标：把多参数的函数处理成每次只处理一个参数的函数，每次都返回一个函数处理剩余的参数，直到最后一个参数处理完才开始执行。
- **偏函数**
	转换目标：固定一部分的参数，然后返回一个函数，拼接处理固定的参数和剩余的参数。
...
1. 函数组合相比函数返回一个函数创建了一个从右到左的数据流，增加了代码的可读性
   ```js
// 这是由内而外的数据流
...
- 使用 Either 来返回一条有礼貌的消息作为错误的回应
		...
		- > 相比 Maybe(null),优势在与可以使用 Left 自定义错误消息，可以知道错误在哪里,相当于自己 throw 了一个自定义的 Error 出去，但是是通过 return 的方式。
