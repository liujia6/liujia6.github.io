...
- 二进制协议
	- HTTP/1.1 版的头信息肯定是文本（ASCII 编码），数据体可以是文本，也可以是二进制。HTTP/2 则是一个彻底的二进制协议，头信息和数据体都是二进制。
		- 二进制数据可以直接由计算机硬件处理，而无需像文本格式那样进行复杂的解析，提高解析效率
		- 客户端还可以指定数据流的优先级。优先级越高，服务器就会越早回应。
- 多路复用
	- 帧（Frames）：最小的通信单位：这些帧可以是不同类型的，如 HEADERS、DATA、PUSH_PROMISE 等，并且它们可以在单个 TCP 连接上交错发送，无需等待其他帧的完成。
	- 流（Streams）：HTTP/2 通过流来管理这些帧的发送和接收。每个请求或响应都对应一个流，并且每个流都有一个唯一的标识符。多个请求和响应可以在同一个 TCP 连接上交错发送，而不会相互干扰。从而实现多路复用。这意味着即使某个请求因为某种原因被延迟，其他请求也可以继续发送和接收数据，从而大大提高了性能
	...
	- 优先级（Priorities）：HTTP/2 允许客户端为每个流设置优先级。当网络拥塞或服务器资源有限时，服务器可以根据这些优先级来决定先处理哪些流。这有助于确保重要的请求得到优先处理。
		- 浏览器资源优先级是内置固定策略，由厂商定义，开发者无法自由自定义。我们只能通过 as、资源标签、加载策略，告诉浏览器资源的真实用途，让浏览器把它放进正确的调度队列，分配正确的优先级
- 服务端推送
...
- [传输编码与内容编码](https://blog.csdn.net/swt369/article/details/77847896)
	- 内容编码（压缩格式）
		- Content-Encoding / Accept-Encoding：内容压缩格式（压报文实体数据，业务数据层面
	- 传输编码（分块编码）
		- Transfer-Encoding / TE：分块传输编码（改报文传输格式，传输层层面）
	- 具体
		- Accept-Encoding（请求头）
			- 作用：客户端告诉服务器「我支持哪些内容压缩算法」
			- 常见值：`gzip, deflate, br`
		- Content-Encoding（响应头）
			- 作用：服务器告诉客户端「当前响应体用了哪种内容压缩」，是对 Accept-Encoding 的应答 TE（请求头）
		- Transfer-Encoding（TE，请求头）
			- 作用：客户端告诉服务器「我支持哪些传输编码方式」
			- 最常用值：chunked（分块传输）
		- Transfer-Encoding（响应头）
			- 作用：服务器告知客户端「本次响应采用什么传输编码」
			- 响应没有 Content-Length，数据分成一块块持续下发，流式接收
- 与 content-length 关系
	- Content-Length 代表数据大小已知，是整包发送。
	- Transfer-Encoding: chunked 代表数据大小未知，是切块流式发送。
...
- 20x
	- 206 partial-content
		- 常用于断点续传，要和传输编码区分开来，这个是知道内容总长度的，有 content-length 的，TE 是不知道的用于直播和流式请求
			- 只有客户端发送了范围请求（`Range` 请求头），服务端才会返回 206
- 30x 重定向
	- 最早只有 302，协议规定 POST 不能变 GET。
		- 但所有浏览器私自改成「POST → GET」，造成标准混乱。
		- HTTP/1.1 拆分出两个新状态码：
	- 303：明确「必须转 GET」，用于页面跳转。
	- 307：明确「不许改方法」，用于接口跳转。
...
>依靠数字证书校验服务端公钥合法性与服务器身份，再借助非对称密码体系（RSA/ECDHE）安全协商出单次会话的对称密钥；握手完成后，业务数据全部使用该对称密钥加密传输。
...
- 利用了证书的数字签名（私钥加密，公钥解密）进行了身份验证，以及数字摘要（常见例如哈希算法）进行完整性校验
	- 应该是利用证书的数字签名（私钥加密，正确的真正属于该服务器的公钥才能解密且一致）
- 使用证书的非对称加密（公钥加密、私钥解密），安全的协商出对话秘钥的算法
- 使用这个协商出来的对话秘钥利用对称加密来加密后续的通话。
...
#### HTTPS-TLS 完整原理（含证书、非对称、签名、对称、验签，面试完整版）

先一句话：

TLS 夹在 TCP 与 HTTP 中间；解决三件事：防窃听、防篡改、防中间人冒充服务器。

靠：非对称加密 + 数字证书 + 数字签名 + 对称加密 组合实现。

##### 一、基础概念拆分

###### 1. 两种加密

1）对称加密（AES-GCM/ChaCha20）

- 同一个密钥加解密，速度极快，适合加密 HTTP 大报文
- 缺点：密钥没法在公网安全传递，不能直接用来初次通信

>用途：握手成功后，正式传输所有 HTTP 数据

2）非对称加密（RSA / ECC）一对公私钥

- 公钥：公开；私钥：服务器自己密存绝不外传
- 规则：公钥加密→只能私钥解密；私钥签名→公钥验签
- 缺点：运算极慢，只适合小数据（随机数、签名）
...
1. 原文做 SHA256 得到摘要
2. 服务端用私钥加密摘要 = 签名
3. 客户端拿公钥解密签名得到原始摘要，再自己对原文哈希对比

- 一致 = 没被篡改；不一致 = 数据被中间人改了

>签名不用加密原文，只加密哈希摘要，速度快。

###### 3. 数字证书（解决公钥被劫持替换问题）

痛点：没有证书时，中间人可以伪造公钥骗客户端。

证书 = CA（权威机构）帮服务器背书：

`域名 + 服务器公钥 + CA信息 + CA用自己私钥做的签名 + 有效期`

- CA 是可信第三方（阿里云、Let’s Encrypt、各大根证书厂商）
- CA 用 CA 私钥对证书内容签名；客户端内置 CA 根公钥用来验证书真伪。

>证书本质：权威机构盖章证明「这个公钥确实属于当前域名网站」

#### HTTPS 区别

- HTTP：明文传输，基于 TCP 80 端口，无加密，返回数据全部明文裸奔，中间人可篡改返回内容
- HTTPS = HTTP + SSL/TLS 加密，基于 TCP 443 端口，握手后对称加密传输，抓包只能看见密文，中间人无法窃听、无法篡改
...
- 强缓存指定过期时间
	...
	- cache-control：max-age=3600，该值优先
		1. no-cache：要求缓存必须在使用前向服务器验证资源的有效性。即使缓存存在，也需要重新验证。 示例：_Cache-Control: no-cache_。
		2. no-store：完全禁止缓存，客户端和代理服务器都不存储请求或响应的任何内容。 示例：_Cache-Control: no-store_。
		3. public：允许任何缓存（包括代理服务器）存储响应内容。 示例：_Cache-Control: public, max-age=86400_。
		4. private**：仅允许单个用户的本地缓存存储内容，代理服务器不能缓存。 示例：_Cache-Control: private, max-age=3600_。
			1. 个人私有数据加 `private` 防止 CDN 共享缓存
		- 场景
			- 用于部分明确什么时候更新的数据
			- 对于不会再更新的数据，或者实时性要求不强的数据，可以设置一个超长的时间，http1.1 规定不能超过 1 年以上
			- 比如静态资源文件的文件名是 hash 值，那么就不需要服务端 etag 控制了，直接设置缓存时间的一个非常长的时间
	1. max-age=：设置缓存的最大存储时间（以秒为单位）。在此时间内，缓存被视为有效。 示例：_Cache-Control: max-age=3600_ 表示缓存有效期为 1 小时。
- 协商缓存
	- （客户端请求头）if-modify-since
		- last-modify（服务端响应）
	- if-none-match
		- etag
- vary 首部，指定缓存单位
...
前端缓存主要靠 HTTP 缓存头配合打包哈希文件名，分五类场景配置：

1. 带哈希的静态资源（js/css/ 图片 / 字体）：文件内容变哈希就变，配置 `Cache-Control: public,max-age=31536000,immutable` 一年强缓存，有效期内直接读本地缓存无网络请求，更新后页面引用新哈希文件自动拉取新版本。
2. 入口 index.html：不能长缓存，配置 `no-cache` 并携带 ETag/Last-Modified 走协商缓存，每次访问都会校验文件更新，避免缓存旧页面加载过期静态资源。
3. 后端接口：实时订单、聊天等强实时接口用 `no-store,no-cache` 禁用缓存；个人私有数据加 `private` 防止 CDN 共享缓存；字典、枚举这类极少变动接口可设置一年长缓存，更新时拼接版本参数刷新缓存；普通列表数据配置短 max-age 做短时缓存减轻服务压力。
4. SSR 页面：公共通用页面用 `s-maxage` 单独给 CDN 设置缓存时长；个人个性化页面仅允许浏览器短时私有缓存。
5. 临时下载、动态海报类资源：直接禁用缓存，保证每次获取最新文件。

另外区分两个关键头：`max-age` 控制浏览器本地缓存，`s-maxage` 只作用 CDN / 代理；`no-cache` 是使用缓存前必须校验，`no-store` 完全不存储任何响应缓存。
