...
- 利用用户在当前网站的认证信息，伪造和诱导用户向恶意第三方网站发起请求执行违法操作
...
由于 AJAX 是不支持跨域发送请求的，所以我们只能够用支持跨域请求的 img、script、form 表单来发起请求，这些请求的限制性很强，不属于 AJAX/fetch 异步请求，浏览器不会发起 OPTIONS 预检，也不会应用 CORS 响应头校验；
...
4. 保证网络请求由真实用户发出
	- 用户操作限制（验证码）
5. 不使用 cookie，使用 token 校验用户身份，换用 token 标识用户身份，放到请求头里面

#### 防护

- 使用 JWT 放在请求头里，避免依赖 Cookie，这是前后端分离最常用方案。
- 重要操作增加验证码，从根源杜绝冒充请求。
- Cookie 设置 SameSite 属性，禁止跨站自动携带 Cookie。

### [XSS](https://juejin.cn/post/6844903685122703367#heading-13)

Cross-site scripting，跨站脚本攻击。

**可控用户输入未经正确转义 / 过滤，被浏览器当成 HTML、JS、CSS 执行**，用来偷 Cookie、劫持页面、钓鱼、窃取用户信息。
...
1. 存储型 XSS：也叫持久性 XSS。2. 常见于评论、私信、发博客。所有查看了该博客该评论的用户，都会执行这段恶意脚本 3. XSS 蠕虫：具有自我传播能力的 XSS 攻击，在恶意脚本中利用用户的登录状态进行关注、发微博、发私信等操作，发出的微博和私信可再带上攻击 URL，诱导更多人点击，不断放大攻击范围。这种窃用受害者身份发布恶意内容，层层放大攻击范围的方式，被称为 "XSS 蠕虫 "。
2. 反射性 XSS：也叫非持久性 XSS 2. 恶意代码来自 URL 参数，前端未过滤直接渲染导致，攻击者诱导用户点击 `https://example.com/search?q=<script>alert('XSS');</script>`
3. Dom 型 XSS：`前端` 取出 `URL` 中的恶意代码执行，通过修改页面的 dom 节点形成的 XSS，称为 dom based XSS

- DOM 型 （漏洞前端） = 前端取出和执行恶意代码（前端不安全渲染（innerHTML/v-html/dangerouslySetInnerHTML））
- 反射型 （漏洞后端） = 后端直接把参数拼进 HTML 并且返回渲染 →
- 存储 （漏洞后端） = 后端存储了不安全的输入拼进 HTML 并且返回渲染
...
- 转义应该在输出 HTML 时进行，而不是在提交用户输入时。
	- 主要是在 innerHTML 渲染（必须用 HTML 就用 DOMPurify 过滤），document.write，href、src 等属性，onClick 内联事件等要注意转义
	- 常见于 通过 URL 传递参数 的功能，如网站搜索、跳转等
	- 关键都是渲染时转义
